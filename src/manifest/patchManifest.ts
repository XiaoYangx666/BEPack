...
export async function patchManifest(options: PatchManifestOptions) {
    ...
    const bp = patchBpManifest(
        bpExisted ? await readJsonFile(bpPath) : createBpManifest(options.config),
        options.config,
    ...
    if (options.config.packs.rp) {
        ...
        const rp = patchRpManifest(
            rpExisted ? await readJsonFile(rpPath) : createRpManifest(options.config),
            options.config
        );
