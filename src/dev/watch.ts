...
import { bpManifest, bpRoot, distRoot, rpManifest, rpRoot, slash, srcEntry } from "../utils/path.js";

export function watchProject(cwd: string, config: ResolvedConfig, logger: Logger, copyTarget?: string) {
    ...
    const ignored = ["node_modules", relative(distRoot(cwd, config)), ".git", relative(path.join(bpRoot(cwd, config), "scripts")), relative(bpManifest(cwd, config)), relative(rpManifest(cwd, config))];
    ...
    watcher.on("all", async (_event, file) => {
        ...
        try {
            ...
            if (copyTarget || config.dev.copy) {
                const target = copyTarget ?? (typeof config.dev.copy === "string" ? config.dev.copy : undefined);
                await copyPacks(cwd, config, target, false, logger);
            }
            logger.done("dev", `updated after ${path.normalize(file)} in ${logger.formatDuration(Date.now() - start)}`);
        } catch (error) {
