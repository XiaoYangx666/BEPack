...
export async function commandDev(options: any) {
    ...
    const { cwd, config } = await loadConfig({ command: "dev", cwd: options.cwd ?? process.cwd(), configPath: options.config });
    logger.clear();
    logger.bepack("dev", `target ${config.target}`);
    logger.progress("dev", "initial build started");
    await runBuild({ cwd, config, logger, typecheck: config.build.typecheck, quiet: Boolean(options.json || options.silent) });
    if (!options.skipCopy && (options.copy || options.copyTarget || config.dev.copy)) {
        const target = options.copyTarget ?? (typeof config.dev.copy === "string" ? config.dev.copy : undefined);
