...
import type { ResolvedDependency } from "./resolveDependencies.js";

type PackageJson = Record<string, any>;

export async function patchPackageJson(cwd: string, config: ResolvedConfig, deps: Record<string, ResolvedDependency>, dryRun = false) {
    const file = path.join(cwd, "package.json");
    if (!(await pathExists(file))) throw new BePackError("PACKAGE_JSON_NOT_FOUND", "package.json not found.", { details: { cwd } });
