...
export async function runBuild(options: RunBuildOptions) {
    ...
    await patchManifest({ cwd: options.cwd, config: options.config, dryRun: Boolean(options.dryRun), ...(options.resolvedDeps ? { resolvedDeps: options.resolvedDeps } : {}) });
    await runHook("beforeBuild", "build", options.cwd, options.config, options.logger);
    options.logger.manifest("manifest.json updated");
    if (!options.dryRun && options.typecheck !== false) await runTypecheck(root, { quiet: Boolean(options.quiet), useNpx: options.config.build.useNpx });
    ...
    options.logger.rolldown(options.config.build.preserveModules ? "preserve modules build complete" : "bundle complete");
    await runHook("afterBuild", "build", options.cwd, options.config, options.logger);
    const durationMs = Date.now() - start;
    return { script: slash(`${options.config.packs.bp.root}/scripts/main.js`), typecheck: options.typecheck !== false, durationMs };
}
